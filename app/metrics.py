...
def test_against_random(df, cluster_labels, cluster_metrics, metric=None):
    """
    ...
    cluster_metrics['calinski_harabasz_random_ratio'] = cluster_metrics['calinski_harabasz'] / random_dict[
        'calinski_harabasz']
    cluster_metrics['davies_bouldin_random_ratio'] = random_dict['davies_bouldin'] / cluster_metrics['davies_bouldin']
...
def test_model_consistency(X_train, X_test, y_test, model, cols_cat=None):
    """
...
def test_labelling_consistency(cluster_data, cluster_labels, fitted_model, cols_cat=None, metric=None):
    """
...
def get_signal_loss_chi_square(original_data, cluster_labels, chi2_data):
    ...
    for seg in unique_segments:
        ...
        sliced_data, core_cols, sub_cols, percent_retained_seg = data_slicer(seg_data, tgt_cols=tgt_cols, pop_modes=pop_modes)
...
def get_signal_loss_metric(data_encoded, cluster_labels, target_column='clusters'):
    """
    Function to extract signal loss for every cluster or the segment based on the target_columns in the input.
...
def get_all_metrics(cluster_data, cluster_labels, n_clusters, model, fitted_model, algo,
    ...
    metrics = test_against_random(cluster_data, cluster_labels, metrics, metric=metric)
    ...
    if hasattr(model, 'predict') and not isinstance(model, BayesianGaussianMixture):
        ...
        if cols_cat:
            metrics['model_consistency'] = test_model_consistency(X_train, X_test, y_test, model_for_testing, cols_cat)
        else:
            metrics['model_consistency'] = test_model_consistency(X_train, X_test, y_test, model_for_testing)
    else:
    ...
    if fitted_model is None or isinstance(model, BayesianGaussianMixture):
        ...
        try:
            metrics['label_consistency'] = test_labelling_consistency(cluster_data, cluster_labels, fitted_model,
    ...
    ml_signal_loss = get_signal_loss_metric(data_encoded=data_encoded.copy(), cluster_labels=cluster_labels,
    ...
    signal, core_columns, optimal_signal, percent_retained = get_signal_loss_chi_square(original_data=full_data.copy(),
