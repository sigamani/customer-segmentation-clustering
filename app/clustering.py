...
from feature_selection import famd
try:
...
class Clusterings:
    ...
    def get_chisquare_data(self,
        ...
        try:
            data_and_labels = pd.DataFrame({'data': rename_segments.reset_index(drop=True),
    ...
    def standarize_data(self, data):
        ...
        if self.categorical_cols and self.method_kprototypes_or_kmodes == 'kprototypes':
            ...
            std_data = scaler.fit_transform(data[num_cols])
    ...
    def kmeans_and_pca(self, data, k, n_components=None, seed=42):
        ...
        if isinstance(data, pd.DataFrame):
            data = self.standarize_data(data)
        if n_components:
            pca_data, n_components = self.get_pca_data(data, find_components=False, n_components=n_components)
        else:
            pca_data, n_components = self.get_pca_data(data)
        kmeans = KMeans(n_clusters=k, random_state=seed)
        try:
            kmeans.fit_predict(pca_data)
        except ValueError:
            pass
        return kmeans, n_components, pca_data

    def kmeans_and_famd(self, data, k, n_components=None, seed=42):
        """
        Method to perform kmeans on pca transformed data.
        :param data: pd.DataFrame of response data
        :param k: Int. Number of clusters
        ...
        :return: Fitted Kmeans model, number of components, and PCA transformed data
        """
        famd_data = famd.FAMD_(data, n_components)
        kmeans = KMeans(n_clusters=k, random_state=seed)
        try:
            kmeans.fit_predict(famd_data)
        except ValueError:
            pass
        return kmeans, n_components, famd_data
    ...
    def get_pca_and_kmeans(self, data, k, n_components=None, seed=42):
        ...
        """
        kmeans, n_components, pca_data = self.kmeans_and_famd(pd.DataFrame(data), k, n_components=n_components, seed=seed)
        metrics = get_cluster_metrics(pca_data, kmeans.labels_, k, n_seed=seed)
        return kmeans, metrics, pca_data
    ...
    def kmeans_and_pca_inertia(self, num_clusters, cluster_df, verbose_binary, n_components):
        ...
        :return: float of clustering cost and dict of cluster metrics
        """
        kmeans, n_components, pca_data = self.kmeans_and_pca(cluster_df, num_clusters, n_components=n_components)
    ...
    def optimal_clusters(self, k_increment=1, method=None, cluster_df=None, hierarchical=False):
        ...
        if method == "kprototypes":
            ...
            for i in inputs:
                print(i)
                parallel_processed.append(self.kmeans_and_pca_inertia(i, cluster_df=cluster_df, verbose_binary=1, n_components=None))
            # parallel_processed = Parallel(n_jobs=self.num_cores)(
    ...
    def kmeans_and_pca_clustering(self, hierarchical=None, mode='eval'):
        ...
        if isinstance(hierarchical, pd.DataFrame):
            df = self.standarize_data(hierarchical)
        else:
            df = self.standarize_data(self.data_encoded)
        try:
        ...
        ranked_metrics = rank_cluster_metrics(metrics_from_clusters)
        best_seed_clustering = ranked_metrics['rank_sum'].idxmax()
        optimal_kmeans, metrics, pca_data = kmeans_and_metrics[best_seed_clustering]
        if isinstance(hierarchical, pd.DataFrame):
        ...
        if mode == 'tune':
            ...
            metrics = get_all_metrics(pca_data, optimal_kmeans.labels_, metrics['n_clusters'], optimal_kmeans,
                                      ...
                                      chi2_data=chi2_data, full_data=self.full_data, data_encoded=self.full_data_encoded)
    ...
    def lca_with_timer(self, hierarchical=None):
        ...
        longer than time decared in the lca() function then it will exit with an RRuntimeError.
    ...
    def lca(self, test_wait_time=False):
        ...
        def pass_to_lca():
            try:
                ...
                lca_return_obj = {'model': 'poLCA model', 'labels': 'lca timed out', 'metrics': 'lca timed out'}
    ...
    def bmm(self, hierarchical=None):
        ...
        metrics = get_all_metrics(self.full_data_encoded[self.cluster_vars], best_bmm_labels,
                                  ...
                                  data_encoded=self.full_data_encoded.copy(deep=True), n_seed=best_bmm_model.random_state)
    ...
    def rules_based(self, q_codes):
        ...
        for q_code in q_codes:
            ...
            chi2_data = self.get_chisquare_data(cluster_labels, rename_segments=cluster_column_unencoded,
                                                clustered_df=full_data_copy, weights=self.weight_column)
            metrics = get_all_metrics(data_encoded_copy[self.cluster_vars], cluster_labels, len(np.unique(cluster_labels)),
                                      model=None, fitted_model=None, algo=f'rules-based - {q_code}', chi2_data=chi2_data,
